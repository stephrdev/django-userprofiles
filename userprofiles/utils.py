...
from userprofiles.settings import USERPROFILES_USE_PROFILE
...
class InvalidRegistrationFormException(ImproperlyConfigured):
    pass

def get_registration_form(path):
    i = path.rfind('.')
    module, attr = path[:i], path[i+1:]
    try:
        mod = import_module(module)
    except ImportError, e:
        raise InvalidRegistrationFormException(
